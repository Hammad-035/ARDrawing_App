      ...
      {device != null && cameraOpened && hasPermission ? (
        <>
          <View
            ...
            <DraggableZoomableImage
              resizeMode={'contain'}
              imageSource={{uri: `${main_item.main_cat_name === 'Custom' ? item?.link : link2}`}}
        ...
        <>
          <View
            style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
            <Image
              source={{uri: `${main_item.main_cat_name === 'Custom' ? item?.link : link2}`}}
              style={{height: hp(55), width: wp(100), borderRadius: 15}}
            />
          </View>
        </>
      )}
